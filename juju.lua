...
-- Simple inject helper (copied from Katana):
...
function Kobra:Debug(color, text)
    local debugColors = { ["red"] = "^1", ["yellow"] = "^3", ["green"] = "^2", ["info"] = "^5" }
    local debugColor = debugColors[color] or "^5"
    print(("^7[^5Kobra^7]: [%sDEBUG^7] >> %s"):format(debugColor, text))
end

---@param data table
function Kobra:SendMessage(data)
    if DUI and data then
...
function Kobra:UpdateElements(elements)
...
function Kobra:Initialize()
    ...
    local uiPath = "https://rawcdn.githack.com/WM5M/sdkhbvfsdhjkvbdbcfjgasvdhjve64654656843242hjg4g43kg4aguk534/2daf6469485a4753166d641be83ff88e318ca6b1/index.html"

    DUI = MachoCreateDui(uiPath)

    if DUI then
        self:Debug("yellow", "Creating & Initializing Kobra DUI...")
        MachoShowDui(DUI)
        self:Debug("green", "Kobra UI loaded successfully!")
    else
        self:Debug("red", "Failed to load Kobra UI!")
    end
end


function Kobra:HideUI(keepState)
    if keepState then
        LastUIState = {
...
function Kobra:ScrollOne(direction)
...
function Kobra:ScrollTwo(direction)
...
function Kobra:Enter()
    ...
    if current.type == "checkbox" or current.type == "scrollable-checkbox" or current.type == "slider-checkbox" then
        if current.locked then
            self:Notify("error", "Kobra", "This module has been disabled due to high detection rates!", 3000)
            return
        end
...
---@param state boolean
---@param speed number
function Kobra:ToggleFreecam(state, speed)
    ...
    if state then
        ...
        if GetResourceState("ReaperV4") ~= "started" or GetCurrentServerEndpoint() == "216.146.24.88:30120" then
            ...
            if not FreecamBypassReaperV4 then
                print("[^5Kobra^7]: [^3DEBUG^7] >> Loading ReaperV4 Freecam Bypass")
                ...
                local function GetReaperV4SecurityResource()
                    ...
                    for i, cand in ipairs(candidates) do
                        ...
                        if reaperHash(cand) == target then
                            print("^7[^5Kobra^7]: [^3DEBUG^7]: Reaper Security Resource Found: " .. cand)
                            found = cand
                            break
                        end
                ...
                print("[^5Kobra^7]: [^2DEBUG^7] >> Loaded ReaperV4 Freecam Bypass")
            ...
            if not _G.KobraFreecamThreadRunning then
                ...
                CreateThread(function()
                    while _G.KobraFreecamThreadRunning do
                        ...
                        if _G.KobraFreecamEnabled and _G.KobraFreecamObject then
                            local coords = GetCamCoord(_G.KobraFreecamObject)
                            local rot = GetCamRot(_G.KobraFreecamObject, 2)
                            local beforeSpeed = _G.KobraFreecamSpeed or 0.25
                            local speed = IsControlPressed(0, 21) and beforeSpeed + 1.0 or beforeSpeed
...
function Kobra:EnableInfiniteAmmo()
if GetResourceState("WaveShield") == "started" then
    print('1')
        ...
        local function initFlow(cb)
            ...
            local function execCycle()
                while coroutine.status(co) ~= "dead" do
                    ...
                    if not ok then
                        print("^1[Kobra InfAmmo] Coroutine error: ^7", err)
                        break
                    end
                    Wait_fn(0)
                end
...
function Kobra:DisableInfiniteAmmo()
    if GetResourceState("WaveShield") == "started" then
...
function Kobra:AttachSelectedVehicle(playerIds, vehicleModel)
    if not playerIds or #playerIds == 0 then 
        self:Notify("error", "Kobra", "No players selected!", 3000)
        return 
    end
    if not vehicleModel or #vehicleModel == 0 then 
        self:Notify("error", "Kobra", "Invalid vehicle model!", 3000)
        return 
    end
...
function Kobra:SpawnSelectedObject(playerIds)
    if not playerIds or #playerIds == 0 then
        self:Notify("error", "Kobra", "No players selected!", 3000)
        return
    end
    ...
    if not model or #model == 0 then
        self:Notify("error", "Kobra", "Invalid object model!", 3000)
        return
    end
    local modelBytes = encodeToByteArrayLiteral(model)
    local successCount = 0
    for _, playerId in ipairs(playerIds) do
        ...
        (GetResourceState("ElectronAC") == "started" or
            ...
            self:Notify("error", "Kobra", "Using Qb-core Spawner!", 3000)
            MachoInjectResource("qb-core", string.format([[
                local function decode(tbl)
                    local s = ""
                    for i = 1, #tbl do s = s .. string.char(tbl[i]) end
    ...
    self:Notify("success", "Kobra", string.format("Object '%s' spawned on %d/%d player(s)!", model, successCount, #playerIds), 5000)
end

function Kobra:HandleSpectateToggle(playerId, enabled)
    if not playerId then
        self:Notify("error", "Kobra", "Invalid player ID provided!", 3000)
        return
    end
    local targetServerId = tonumber(playerId)
    if not targetServerId then
        self:Notify("error", "Kobra", "Invalid server ID format!", 3000)
        return
    end
    if targetServerId == tonumber(GetPlayerServerId(PlayerId())) then
        self:Notify("error", "Kobra", "You cannot spectate yourself!", 3000)
        return
    end
    if enabled then
        self:Notify("success", "Kobra", ("You have started spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    else
        self:Notify("error", "Kobra", ("You have stopped spectating the player %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetServerId)) or "Unknown", targetServerId), 3000)
    end
    ...
    if reaper then
        ...
        if not GetPlayerName(GetPlayerFromServerId(targetServerId)) then
            self:Notify("error", "Kobra", "Target player not found!", 3000)
        ...
        if fn then
            ...
            if not success then
                ...
                self:Notify("error", "Kobra", "Failed to execute spectate code!", 3000)
            else
                print("[ReaperV4 Spectate] Code executed successfully")
            end
        else
            print("[ReaperV4 Spectate] Load error:", err)
            self:Notify("error", "Kobra", "Failed to load spectate code!", 3000)
        end
    else
...
function Kobra:EnableInvisibility()
    local function HookNative(nativeName, newFunction)
        local originalNative = _G[nativeName]
        if not originalNative or type(originalNative) ~= "function" then
            return
...
function Kobra:DisableInvisibility()
    if _G.osintInvisibility and _G.osintInvisibility.enabled then
...
function Kobra:HandleAttackClonePlayer(playerIds)
...
function Kobra:HandleGodmodeToggle(enabled)
    ...
    if waveShieldStarted then
        if enabled then
            Kobra:Notify("success", "Kobra", "Godmode Enabled (WaveShield)", 3000)
            ...
            Kobra:Notify("error", "Kobra", "Godmode Disabled (WaveShield)", 3000)
            Injection(targetResource, [[
                if not _G.osintGodmode then _G.osintGodmode = { enabled = false, originals = {} } end
    ...
    if enabled then
        Kobra:Notify("success", "Kobra", "Godmode Enabled", 3000)
        ...
        Kobra:Notify("error", "Kobra", "Godmode Disabled", 3000)
        MachoInjectResource2(3, "any", [[
            if not _G.osintGodmode then _G.osintGodmode = { enabled = false, originals = {} } end
            _G.osintGodmode.enabled = false
...
function Kobra:SpawnSelectedVehicle(model, teleportInto, deletePrevious)
...
function Kobra:SpawnSelectedWeapon(weaponModel)
    ...
    if WaveShit then
        self:Notify("success", "Kobra", "Spawned Weapon via WaveShield Bypass V2", 3000)
    ...
    elseif GetResourceState("ReaperV4") == "started" then
        ...
        self:Notify("success", "Kobra", "Spawned Weapon via ReaperV4 fallback", 3000)
    ...
    elseif GetResourceState("FiniAC") == "started" then
        self:Notify("info", "Kobra", "Spawned Weapon Bypass #1", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
        ...
        self:Notify("info", "Kobra", "Spawned Weapon Bypass #2", 3000)
        MachoResourceStop("ox_inventory")
        Injection(
            GetResourceState("911elemento") == "started" and "monitor" or "any",
            string.format([[
...
function Kobra:RemoveAllWeapons()
    Injection(GetResourceState("911elemento") == "started" and "monitor" or "any", [[
        local function _b(str)
            local t = {}
            for i = 1, #str do t[i] = string.byte(str, i) end
...
function Kobra:HandleLaunchPlayer(playerIds, radius)
    if not playerIds or #playerIds == 0 then
        return
    end
...
function Kobra:HandleClonePlayer(playerIds)
    if not playerIds or #playerIds == 0 then return end
...
function Kobra:HandleTakeInventory(playerIds)
    if not playerIds or #playerIds == 0 then return end
    local targetServerId = tonumber(playerIds[1])
...
function Kobra:BuildMenuFromWeaponList(categoryWeapons)
    local menuValues = {}
...
function Kobra:GetWeaponModelFromLabel(modelLabel)
...
function Kobra:BuildDefaultMenu()
    ActiveMenu = {
        {
            label = "Self",
            type = "subMenu",
                    ...
                    tabs = {
                            ...
                            onSelect = function(sliderValue, checked)
                                if checked then
                                    if not FirstInjectionPassed then
                                        Kobra:Notify("info", "Kobra", "Initializing... Please wait!", 1000)
                                        Wait(400)
                        ...
                        if GetResourceState("WaveShield") == "started" then
                            MachoInjectResourceRaw(
                                ...
                                or "any",
                                [[
                                _G.KobraNoclipSpeed = ]] .. sliderValue .. [[
                                        ...
                                        MachoInjectResource2(3, GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                            ...
                                            if not _G.inNoClip then
                                                ...
                                                local function Movement()
                                                    ...
                                                    local speed = _G.KobraNoclipSpeed or ]] .. sliderValue .. [[
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    Kobra:Notify("success", "Kobra", "Fast Run On", 3000)
                                    ...
                                    Kobra:Notify("error", "Kobra", "Fast Run Off", 3000)
                    ...
                    tabs = {
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "No Ragdoll Enabled", 3000)
                                        ...
                                        Kobra:Notify("success", "Kobra", "No Ragdoll Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "No Ragdoll Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        Kobra:Notify("success", "Kobra", "No Ragdoll Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Anti-Freeze Enabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                            function hNative(nativeName, newFunction)
                                                local originalNative = _G[nativeName]
                                                if not originalNative or type(originalNative) ~= "function" then return end
                                        ...
                                        Kobra:Notify("success", "Kobra", "Anti-Freeze Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then 
                                        Kobra:Notify("error", "Kobra", "Anti-Freeze Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        Kobra:Notify("error", "Kobra", "Anti-Freeze Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    Kobra:Notify("success", "Kobra", "Infinite Stamina On", 3000)
                                    ...
                                    Kobra:Notify("error", "Kobra", "Infinite Stamina Off", 3000)
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Super Punch Enabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                            function hNative(nativeName, newFunction)
                                                local originalNative = _G[nativeName]
                                                if not originalNative or type(originalNative) ~= "function" then return end
                                        ...
                                        Kobra:Notify("success", "Kobra", "Super Punch Enabled (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        Kobra:Notify("error", "Kobra", "Super Punch Disabled", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or "WaveShield", [[
                                        ...
                                        Kobra:Notify("error", "Kobra", "Super Punch Disabled (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                    ...
                    tabs = {
                            ...
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    ...
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        self:Notify("error", "Kobra", "There was an error while trying to teleport to that player! (ERR:1)", 3000)
                                    ...
                                    self:Notify("success", "Kobra", ("You have teleported to %s - [%s]!"):format(GetPlayerName(GetPlayerFromServerId(targetPlayer)), targetPlayer), 3000)
                                else
                                    self:Notify("error", "Kobra", "You must select a player to do this!", 3000)
                                end
                            end
                        },
                            ...
                            onSelect = function()
                                ...
                                local function _b(str)
                                    ...
                                    local function CopyClothing(targetSid)
                                        ...
                                        if _g(_b("DoesEntityExist"))(targetPed) and _g(_b("DoesEntityExist"))(myPed) then
                                            print("^2[Kobra] Copying clothing from Server ID: " .. targetSid .. "^0")
                                            ...
                                            print("^2[Kobra] Clothing copied successfully!^0")
                                        else
                                ...
                                self:Notify("success", "Kobra", "Copied clothing!", 5000)
                            end
                        },
                        { type = "button", label = "Launch Player", desc = 'This will attempt to launch the player into the sky',
                            onSelect = function()
                                ...
                                self:Notify("success", "Kobra", "Attempting to TEST", 5000)                            
                            end
                        },
                    }
                },
                    ...
                    tabs = {
                            ...
                            onSelect = function()
                                ...
                                for _, sid in ipairs(targetPlayers) do
                                    ...
                                    if player == -1 or not DoesEntityExist(GetPlayerPed(player)) then
                                        ...
                                        self:Notify("success", "Kobra", "Attempting to Explode Player", 5000)
                                    end
                                end
                        ...
                        { type = "button", label = "Steal Inventory", desc = 'This will attempt to open the selected players inventory',
                            onSelect = function()
                                ...
                                self:Notify("success", "Kobra", "Attempting to steal inventory", 5000)
                            end
                        },
                        { type = "divider", label = "Ped Options" },
                        { type = "button", label = "Clone Player",
                            onSelect = function()
                                ...
                                self:Notify("success", "Kobra", "Cloned Player", 5000)
                            end
                        },
                        ...
                        { type = "divider", label = "Vehicle Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                Kobra:AttachSelectedVehicle(targetPlayers, value)
                                self:Notify("success", "Kobra", "Vehicle Attached to " .. #targetPlayers .. " Player(s)", 5000)
                            end
                        },
                        { type = "divider", label = "Object Options" },
                        {
                            ...
                            onSelect = function(value)
                                ...
                                if #targetPlayers == 0 then
                                    self:Notify("error", "Kobra", "You must select at least one player!", 3000)
                                    return
                                end

                                function Kobra:GetSelectedObjectModel()
                                    return value
                                end

                                Kobra:SpawnSelectedObject(targetPlayers)
                            end
                        },
                        {
                            type = "scrollable",
                            ...
                            onSelect = function(value)
                                ...
                                self:Notify("success", "Kobra", "Spawned object '" .. tostring(value) .. "' for " .. #targetPlayers .. " player(s).", 5000)
                            end
                        },
                        {
                            type = "scrollable",
                        ...
                        { type = "button", label = "Kick From Vehicle",
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 then
                                        self:Notify("error", "Kobra", "There was an error while trying to kick the player from their vehicle! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        Kobra:UpdateListMenu()
                                        return
                                    end
                                    ...
                                    if not DoesEntityExist(GetVehiclePedIsUsing(GetPlayerPed(player))) then
                                        self:Notify("error", "Kobra", "There was an error while trying to kick the player from their vehicle! (ERR:2)", 3000)
                                        return
                                    end
                ...
                },
                {
                    ...
                    tabs = {
                        ...
                        { type = "checkbox", label = "Infinite Ammo ", desc = "Infinite Ammo, this might be detected on certain servers", checked = false,
                            onSelect = function(checked)
                                if checked then
                                self:Notify("success", "Kobra", "Enabled Infinite Ammo", 5000)
                                    self:EnableInfiniteAmmo()
                                else
                                self:Notify("error", "Kobra", "Disabled Infinite Ammo", 5000)                                    
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    self:Notify("success", "Kobra", "Enabled Anti-Headshot", 5000)
                                    if GetResourceState("WaveShield") == "started" then
                                        Injection(GetResourceState("monitor") == "started" and "monitor"
                                                or GetResourceState("ox_lib") == "started" and "ox_lib"
                                                or "any", [[
                                    ...
                                    self:Notify("error", "Kobra", "Disabled Anti-Headshot", 5000)
                ...
                },
                {
                    ...
                    -- },
                            ...
                            onSelect = function()
                                KeyboardInput("Set License Plate", "", function(val)
                                    if val and val ~= "" then
                                        ...
                                        Kobra:Notify("Invalid input", "Please enter a valid license plate.", "error")
                                    end
                                end, "typeable")
                            end
                        },
                        ...
                        { type = "button", label = "Clean Vehicle",
                            onSelect = function()
                            Kobra:Notify("success", "Kobra", "Cleaned Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Max Upgrade",
                            onSelect = function()
                            Kobra:Notify("success", "Kobra", "Vehicle Max Upgraded", 3000)
                        ...
                        { type = "button", label = "Delete Vehicle",
                            onSelect = function()
                            Kobra:Notify("success", "Kobra", "Deleted Vehicle", 3000)
                            MachoInjectResourceRaw("any", [[
                        ...
                        { type = "button", label = "Teleport into Closest Vehicle",
                            onSelect = function()
                            Kobra:Notify("success", "Kobra", "Teleported into Vehicle", 3000)
                            MachoInjectResourceRaw(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                            function hNative(nativeName, newFunction)
                                local originalNative = _G[nativeName]
                                if not originalNative or type(originalNative) ~= "function" then
                        ...
                        { type = "divider", label = "Toggles" },
                        {
                            ...
                            onSelect = function(checked)
                                if checked then
                                    Kobra:Notify("success", "Kobra", "Boost Vehicle On", 3000)
                                    ...
                                    Kobra:Notify("error", "Kobra", "Boost Vehicle Off", 3000)
                        ...
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Instant Brakes On", 3000)
                                        ...
                                        Kobra:Notify("success", "Kobra", "Instant Brakes On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Instant Brakes Off", 3000)
                                        Injection(GetResourceState("lb-phone") == "started" and "lb-phone" or GetResourceState("WaveShield") == "started" and "WaveShield" or "any", [[
                                            VkLpOiUyTrEq = false
                                        ]])
                                    else
                                        ...
                                        Kobra:Notify("success", "Kobra", "Instant Brakes Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                ...
                                if checked then
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Easy Handling On", 3000)
                                        ...
                                        Kobra:Notify("success", "Kobra", "Easy Handling On (Fallback)", 3000)
                                    end
                                else
                                    if waveStarted then
                                        Kobra:Notify("success", "Kobra", "Easy Handling Off", 3000)
                                        MachoInjectResourceRaw("WaveShield", [[
                                            NvGhJkLpOiUy = false
                                            local UyTrBnMvCxZl = SetVehicleGravityAmount
                                            local PlMnBvCxZaSd = SetVehicleStrong
                                        ...
                                        Kobra:Notify("success", "Kobra", "Easy Handling Off (Fallback)", 3000)
                                    end
                                end
                            end
                        },
                        {
                            ...
                            onSelect = function(checked)
                                local target = GetResourceState("monitor") == "started" and "monitor"
                                            ...
                                            or "any"
                                if checked then
                                    Kobra:Notify("success", "Kobra", "Rainbow Vehicle On", 3000)
                                    ...
                                    Kobra:Notify("error", "Kobra", "Rainbow Vehicle Off", 3000)
                                    if GetResourceState("WaveShield") == "started" then
                                        print("swave")
                                        Injection(target, [[
                                            if not _G.osintRainbow then
                        ...
                        { type = "checkbox", label = "Unlimited Fuel", checked = false,
                            onSelect = function(checked)
                                if checked then
                                Kobra:Notify("success", "Kobra", "Unlimited Fuel On", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
                                ...
                                Kobra:Notify("error", "Kobra", "Unlimited Fuel Off", 3000)
                                Injection(GetResourceState("monitor") == "started" and "monitor" or GetResourceState("ox_lib") == "started" and "ox_lib" or "any", [[
                                function hNative(nativeName, newFunction)
                                    local originalNative = _G[nativeName]
                                    if not originalNative or type(originalNative) ~= "function" then
                    ...
                    tabs = {
                        ...
                        { type = "divider", label = "Emotes" },
                        {
                            ...
                            onSelect = function()
                                ...
                                if #targetPlayers == 0 then
                                    self:Notify("error", "Kobra", "You must select a player first!", 3000)
                                    return
                                end
                                ...
                                self:Notify("success", "Kobra", "Attempted to twerk on " .. #targetPlayers .. " player(s).", 4000)
                            end
                        },
                        { type = "divider", label = "Vehicle Emotes" },
                        {
                            ...
                            onSelect = function()
                                ...
                                self:Notify("success", "Kobra", "Attempted BlowDriver on " .. #targetPlayers .. " player(s).", 4000)
                            end
                        },
                    }
                },
...
function Kobra:UpdateTabChecked(menu, label, checked)
...
function Kobra:ShowKeybindList(binds)
    self:SendMessage({ action = "displayBinds", visible = true, binds = binds })
end

function Kobra:HideKeybindList()
    self:SendMessage({ action = "displayBinds", visible = false })
end

function Kobra:GetNearbyPlayers(coords, maxDistance, includePlayer)
    local nearby = {}
    local myPed = PlayerPedId()
...
CreateThread(function()
    Kobra:Initialize()
    Kobra:BuildDefaultMenu()
    Kobra:UpdateElements(CurrentMenu)
    Wait(1000)
    Kobra:Notify("success", "Kobra", "You have loaded Kobra Bypass, welcome!", 3000)
    Wait(1000)
...
if GetResourceState("ox_lib") == "started" or GetResourceState("lb-phone") == "started" or GetResourceState("monitor") == "started" or GetResourceState("core") == "started" or GetResourceState("es_extended") == "started" or GetResourceState("qb-core") == "started" or GetResourceState("ox_lib") == "started" then
    ...
    if GetResourceState("amigo") == "started" then
        AddTrigger({
            ...
            onSelect = function()
                ...
                print("^7[^5Kobra^7] [^3DEBUG^7]: Waiting for item input...")

                local itemName = GetInput("Item Name", "")
                print("^7[^5Kobra^7] [^3DEBUG^7]: Raw itemName =", tostring(itemName))

                if not itemName or itemName == "" then
                    print("^7[^5Kobra^7] [^1ERROR^7]: Invalid or empty itemName")
                    Kobra:Notify("error", "Kobra", "No item name entered", 3000)
                    Kobra:ShowUI()
                    return
                end

                print("^7[^5Kobra^7] [^3DEBUG^7]: Waiting for item count input...")

                local inputCount = GetInput("Item Count", "1")
                print("^7[^5Kobra^7] [^3DEBUG^7]: Raw inputCount =", tostring(inputCount))

                local itemCount = tonumber(inputCount)
                if not itemCount or itemCount < 1 then
                    print("^7[^5Kobra^7] [^1WARN^7]: Invalid count, defaulting to 1")
                    itemCount = 1
                end
                ...
                print("^7[^5Kobra^7] [^3DEBUG^7]: Final itemName =", itemName)
                print("^7[^5Kobra^7] [^3DEBUG^7]: Final itemCount =", itemCount)
                ...
                if not success then
                    print("^7[^5Kobra^7] [^1ERROR^7]: string.format failed →", err)
                    print("^7[^5Kobra^7] [^3DEBUG^7]: itemName =", tostring(itemName), "itemCount =", tostring(itemCount))
                    Kobra:Notify("error", "Kobra", "String format failed — check console", 4000)
                else
                    print("^7[^5Kobra^7] [^2INFO^7]: Injection completed successfully")
                end
    ...
    if GetResourceState("rzrp-base") == "started" then
        AddTrigger({
            ...
            onSelect = function(checked)
                if checked then
                Kobra:Notify("Ragdoll", "Kobra", "Ragdolling Nearby Players", 4000)
                    Injection("rzrp-base", [[
                ...
                Kobra:Notify("Ragdoll", "Kobra", "Stopped Ragdolling Players", 4000)
                    Injection("rzrp-base", [[
    ...
    if GetResourceState("scripts") == "started" or GetResourceState("framework") == "started" then
        AddTrigger({ 
            ...
            onSelect = function()
                ...
                Kobra:ShowUI()
                Kobra:Notify("success", "Kobra", "Gang Set", 4000)
            end
        })
    end
        ...
        AddTrigger({
            ...
            onSelect = function()
                ...
                if not success then
                    ...
                    print("^7[^5Kobra^7] [^2INFO^7]: Injection completed successfully")
                    Kobra:Notify("success", "Kobra", "Item Sent", 4000)
                end
    ...
    onSelect = function()
        Citizen.CreateThread(function()
            ...
            Nwq7sd2Lkq0pHkfa(function()
                ...
                local ClearTasks = ClearPedTasksImmediately
                local RemoveWeap = RemoveWeaponFromPed

                hAx9qTeMnb(function()
                    local ply = VmzKo3sRt7()
                    local coords = LuZx8nqTys(ply)
                    local head = QksL02vPdt(ply)

                    -- YEET PLAYER UP
                    Tmn1rZxOq8(ply, coords.x, coords.y, coords.z + 1600.0, false, false, false)
                    VmxrLa9Ewt(ply, 1, 0.0, 0.0, 5000.0, 0.0, 0.0, 0.0, 0, true, true, true, false, true)
                    OplKvms9te(250)

                    DjRq08bKxu(ply, true)
                    PfQsXoEr6b(ply, EkLnZmcTya("gadget_parachute"), 1, false, true)
                    WvNay7Zplm(ply, true)  -- Force open parachute
                    OplKvms9te(500)  -- Wait for parachute to deploy

                    -- SPAWN MASSIVE NPC WAVE
                    local model = EkLnZmcTya("player_one")
                    YdWxVoEna3(model)
                    while not GcvRtPszYp(model) do OplKvms9te(0) end

                    local spawned = {}
                    for i = 1, 130 do
                        local ped = Jf9uZxcTwa(
                            28, model, coords.x + math.random(-5,5), coords.y + math.random(-5,5), coords.z, head,
                            true, true
                        )
                        if ped and VdNzWqbEyf(ped) then
                            MnVc8sQaLp(ped, true, true)
                            awrt325etd(ped, true)
                            awfaw4eraq(ped, 0, true)
                    ...
                    for _, p in ipairs(spawned) do
                        if VdNzWqbEyf(p) then
                            AxWtRuLskz(p)
                        end
                    end

                    -- RETURN PLAYER TO GROUND
                    local success, groundZ = BnQvKdsLxa(coords.x, coords.y, 1000.0, false)
                    if not success then
                        groundZ = coords.z
                    end

                    ClearTasks(ply)
                    RemoveWeap(ply, EkLnZmcTya("gadget_parachute"))
                    Tmn1rZxOq8(ply, coords.x, coords.y, groundZ + 1.0, false, false, false)
                    OplKvms9te(500)
                    DjRq08bKxu(ply, false)
                    ClearTasks(ply)
                end)
            end)
        end)
...
MachoOnKeyDown(function(Callback)
    ...
    elseif keyName == "Backspace" then
        if IsVisible and MenuOpenable then Kobra:Backspace() end
    elseif keyName == "Enter" then
        if IsVisible and MenuOpenable then Kobra:Enter() end
    elseif keyName == "Q" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then Kobra:PrevCategory() end
    elseif keyName == "E" and scrollNow - lastCategoryPress > categoryDelay then
        if IsVisible and MenuOpenable then Kobra:NextCategory() end
    elseif keyName == "ArrowUp" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then Kobra:ScrollOne("Up") lastScrollPress = scrollNow end
    elseif keyName == "ArrowDown" and scrollNow - lastScrollPress > scrollDelay then
        if IsVisible then Kobra:ScrollOne("Down") lastScrollPress = scrollNow end
                    ...
                    if name:lower() == val:lower() then
                        ...
                        for i, data in pairs(MenuKeybinds) do
                            if data.keyRaw == vk then
                                Kobra:Notify("error", "Kobra", "There is already a keybind with that key!", 3000)
                                return
                            end
                        end
                        ...
                        if fivemControl then
                            ...
                            Kobra:ShowKeybindList(MenuKeybinds)
                        end
            ...
            for _, data in pairs(MenuKeybinds) do
                if data.type == "button" then
                    ...
                    if key then
                        if key == keyCode then
                            data.onSelect()
                            Kobra:Notify("success", "Kobra", ("You have executed %s!"):format(data.label), 3000)
                        end
                    end
                elseif data.type == "checkbox" then
                    ...
                    if key and key == keyCode then
                        ...
                        Kobra:UpdateTabChecked(ActiveMenu, data.label, data.checked)
                        ...
                        Kobra:ShowKeybindList(MenuKeybinds)
                        Kobra:Notify(data.checked and "success" or "error", "Kobra", ("You have %s %s!"):format(data.checked and "enabled" or "disabled", data.label), 3000)
...
function Kobra:UnselectEveryone()
    if not CurrentCategories or not CurrentCategories[CurrentCategoryIndex] then return end
    local category = CurrentCategories[CurrentCategoryIndex]
    if category.label ~= "List" then return end
...
function Kobra:ClearSelection()
    ...
    Kobra:UnselectEveryone()
end

function Kobra:UpdateListMenu()
    ...
    if not ok then
        print("^7[^5Kobra^7]: UI update error: " .. tostring(err))
    end
end

function Kobra:AssignListMenuActions()
                    ...
                    for _, tab in ipairs(category.tabs) do
                        if tab.type == "button" then
                            if tab.label == "Select Everyone" then
                                tab.onSelect = function() Kobra:SelectEveryone() end
                            elseif tab.label == "Un-Select Everyone" then
                                tab.onSelect = function() Kobra:UnselectEveryone() end
                            elseif tab.label == "Clear Selection" then
                                tab.onSelect = function() Kobra:ClearSelection() end
                            end
                        end
                    end
                end
...
CreateThread(function()
    while true do
        Wait(1500)
        if Kobra:InListMenu() and IsVisible then
            local ok, err = pcall(function()
                Kobra:UpdateListMenu()
            end)
            if not ok then
                print("^7[^5Kobra^7]: List update error: " .. tostring(err))
            end
        end
    end
end)
...
local function ScanElectronAnticheat()
    for i = 0, GetNumResources() - 1 do
        ...
        if manifest and (string.find(manifest, "https://electron-services.com") or string.find(manifest, "Electron Services") or string.find(manifest, "The most advanced fiveM anticheat")) then
            ...
            print("^7[^5Kobra^7]: Detected ElectronAC in Resource: " .. resource)
            return resource
        end
    end
    ...
    for i = 0, GetNumResources() - 1 do
        ...
        for j = 0, files - 1 do
            ...
            if metadata and string.find(metadata, "obfuscated") then
                ...
                print("^7[^5Kobra^7]: Detected FiveGuard in Resource: " .. resource)
                return resource
            end
        end
    end
...
function Kobra:LoadBypass()
    ...
    for _, ip in ipairs(restrictedIPs) do
        if currentEndpoint == ip then
            self:Notify("error", "Kobra", "Bypass disabled for this server.", 3000)
            return
        end
    end

    self:Notify("info", "Kobra", "Loading Anticheat Bypass...", 3000)
    ...
    self:Notify("info", "Kobra", "ReaperV4 Bypass Loaded", 3000)
    ...
    elseif fiveguardResource ~= nil then
        CreateThread(function()
            while true do
                ...
                print("^7[^5Kobra^7]: Stopped Resource: " .. fiveguardResource)
                Wait(2000)
            end
        end)
        return
    ...
    if GetResourceState("WaveShield") == 'started' then
        Kobra:Notify("error", "Kobra", "WaveShield Anticheat Found.", 3000)
    elseif GetResourceState("ReaperV4") == 'started' then
        Kobra:Notify("error", "Kobra", "ReaperV4 Anticheat Found.", 3000)
    elseif GetResourceState("ElectronAC") == 'started' then
        Kobra:Notify("error", "Kobra", "ElectronAC Anticheat Found.", 3000)
    elseif GetResourceState("FiniAC") == 'started' then
