...
if not valid then
    MachoMenuNotification("Katana Menu", "Your key ain't valid lmfao: " .. CurrentKey .. " (" .. reason .. ")", 10)
    return
end
Citizen.CreateThread(function()
    if expiresEpoch and expiresEpoch > 0 then
        ...
        if remaining > 0 then
            ...
            MachoMenuNotification("Katana Menu", msg, 5)
        else
            MachoMenuNotification("Katana Menu", "Key expired.", 10)
        end
    else
        MachoMenuNotification("Katana Menu", "Key valid (no expiry field).", 5)
    end
end)
...
local function CheckResource(resource)
 ...
 local function LoadBypasses()
    ...
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Finalizing.")

    Wait(500)

    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Almost there!!!!.")
end
...
Citizen.CreateThread(function()
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "It's now safer.")
    Wait(500)
    MachoMenuNotification("[NOTIFICATION] Katana Menu", "Enjoy!")
end)
...
local function toggleVehicleBoost()
    ...
    if isVehicleBoostEnabled then
        MachoMenuNotification("Katana", "Vehicle boost is ON fucking enjoy it.", 3)
    else
        MachoMenuNotification("Katana", "Vehicle boost is now OFF.", 3)
    end
end
...
local function toggleVehicleUnlocker()
    ...
    if isVehicleUnlockerActive then
        MachoMenuNotification("Katana", "Vehicle Unlocker Enabled. Press E on a vehicle.", 3)
    else
        MachoMenuNotification("Katana", "Vehicle Unlocker Disabled.", 3)
    end
end
        ...
        if player ~= PlayerId() then
            ...
            if DoesEntityExist(ped) then
                if #(GetEntityCoords(ped) - selfCoords) <= 2.5 then
                    ...
                    ExecuteCommand("rob")
                    MachoMenuNotification("Katana", "Tappin Pockets.", 3)
                    return
                end
            end
        end
    end
    MachoMenuNotification("Katana", "Bro There's No one To Rob.", 3)
end
    ...
    if not triggerFinderTab.selectedTrigger then
        table.insert(
            triggerFinderTab.items,
            {
                ...
                action = function(s)
                    MachoMenuNotification("Katana", "Looking For Triggers.", 3)
                    Citizen.Wait(250)
                    ...
                    MachoMenuNotification(
                        "Katana",
                        "There Is " .. #triggerFinderTab.foundTriggers .. " triggers in here.",
                        5
                    )
            ...
            triggerFinderTab.items,
            {
                ...
                action = function(s)
                    ...
                    if
                     ...
                     then
                        MachoMenuNotification("Katana", "Invalid input or amount.", 5)
                        return
                    end
                    ...
                    if handled then
                        MachoMenuNotification("Katana", "Trigger Worked, Enjoy!", 5)
                    else
                        MachoMenuNotification(
                            "Katana",
    ...
    tabs = {
        {
            ...
            items = {
                ...
                {label = "More Servers Coming Soon", action = function()
                        MachoMenuNotification("Katana MeNu", "More Servers Coming Soon.", 5)
                    end}
            }
        },
        {
                ...
                {type = "input", label = "Payload", value = "", placeholder = "Payload Goes Here"},
                {
                    ...
                    action = function(state)
                        ...
                        if not eventName or eventName == "" then
                            MachoMenuNotification("Katana", "The event name cannot be empty.", 5)
                            return
                        end
                        inject(
                        ...
                        MachoMenuNotification("Katana", "Trigger Executed.", 5)
                    end
                },
                {
                    type = "button",
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "Revived.", 5)
                    end
                },
                {
                    type = "button",
                    label = "Revive Player [Safer]",
                    action = function(state)
                        local targetRes =
                            CheckResource("ox_inventory") and "ox_inventory" or
                            CheckResource("ox_lib") and "ox_lib" or
                            CheckResource("es_extended") and "es_extended" or
                            CheckResource("qb-core") and "qb-core" or
                            CheckResource("wasabi_ambulance") and "wasabi_ambulance" or
                            CheckResource("ak47_ambulancejob") and "ak47_ambulancejob" or
                            CheckResource("oxmysql") and "oxmysql" or
                            "any"
                        MachoInjectResource2(3, targetRes, [[
                        ...
                        MachoMenuNotification("Katana", "Revive Triggered!", 5)
                    end
                },
                {
                    type = "button",
                    label = "Rob Inventorys [V2]",
                    action = function()
                        isRobberyActive = not isRobberyActive
                        MachoMenuNotification(
                            "Katana",
                            isRobberyActive and "Do /steal or /rob to rob." or
                                "You're no longer tappin pockets.", 7
                        )
                    ...
                    action = function(state)
                        ...
                        if not ActiveInventory then
                            MachoMenuNotification("Katana", "No ox_inventory or qb-inventory found.", 5)
                            return
                        end
                        -- === STEP 3: INJECT INTO FIRST ox_* RESOURCE ===
                        local Resources = {
                            "ox_inventory", "ox_doorlock", "ox_fuel", "ox_target",
                            "ox_lib", "ox_sit", "ox_appearance"
                        }
                        ...
                        -- === FEEDBACK ===
                        if injected then
                            MachoMenuNotification("Katana", "Force Rob ACTIVE! Press [E] near player.", 7)
                        else
                            MachoMenuNotification("Katana", "Failed: No ox resource to inject.", 5)
                        end
                    end
                },
                {
            ...
            items = {
                {
                    ...
                    action = function(state)
                        MachoMenuNotification("Katana", "Enabled, press again to scan.", 7)
        ...
        },
        {
            ...
            type = "form",
            items = {
            {type = "button", label = "Crash Server", action = function()
MachoInjectResource((MachoResourceInjectable("ReaperV4") and "ReaperV4") or
(MachoResourceInjectable("FiniAC") and "FiniAC") or
(MachoResourceInjectable("WaveShield") and "WaveShield") or
(MachoResourceInjectable("monitor") and "monitor") or "any", [[
...
-- Execute the main function
                    ...
                    MachoMenuNotification("Katana MeNu", "Crashing, please wait..", 5)
                end}
            }
        },
            ...
            items = {
                {
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "ESP " .. (espEnabled and "on" or "off"), 5)
                    end
                },
                {
                    type = "button",
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "Self Skeleton " .. (showSelfSkeleton and "ON" or "OFF"), 5)
                    end
                },
                {
                    type = "button",
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "Player Names " .. (namesEnabled and "ON" or "OFF"), 5)
                    end
                },
                {
                    type = "button",
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "Player IDs " .. (idsEnabled and "ON" or "OFF"), 5)
                    end
                },
                {
    type = "button",
    label = "Attach To Player (E)",
    action = function()
                        ...
                        if not attached then
                            ...
                            for _, pid in ipairs(GetActivePlayers()) do
                                if pid ~= PlayerId() then
                                    local ped = GetPlayerPed(pid)
                                    if DoesEntityExist(ped) then
                                        local d = #(GetEntityCoords(me) - GetEntityCoords(ped))
                                        if d < dist then
                                            dist = d
                                            closest = ped
                                        end
                            ...
                            if closest ~= -1 then
                                target = closest
                                local bone = GetPedBoneIndex(target, 57005)
                                AttachEntityToEntity(me, target, bone,
                                    0.0, 0.0, 0.0,
                                    0.0, 90.0, 0.0,
                                    false, false, false, false, 2, true)
                                attached = true
                                print("^2[Katana] Attached to player!")
                            else
                                print("^3[Katana] No player in 4m")
                            end
                        else
                            DetachEntity(me, true, false)
                            attached = false
                            target = nil
                            print("^2[Katana] Detached")
                        end
                    end
                end
            end)
        ]])
        MachoMenuNotification("Katana", "Attach script loaded! Press E near player.", 5)
    end
},
                {
                    type = "button",
                    ...
                    action = function()
                        ...
                        MachoMenuNotification("Katana", "Crosshair " .. (crosshairEnabled and "ON" or "OFF"), 5)
                    end
                }
            }
        },
                ...
                if isVehicleUnlockerActive then
                    ...
                    if DoesEntityExist(vehicle) then
                        ...
                        if locked == 1 then
                            ...
                            MachoMenuNotification("Katana", "Vehicle locked.", 3)
                        else
                            SetVehicleDoorsLocked(vehicle, 1)
                            SetVehicleDoorsLockedForAllPlayers(vehicle, false)
                            MachoMenuNotification("Katana", "Vehicle unlocked.", 3)
                        end
                    else
                        MachoMenuNotification("Katana", "No vehicle nearby, stupid ass.", 3)
                    end
                end
                ...
                if isCtrlPressed then
                    ...
                    elseif key == 0x43 then
                        if item.value and item.value ~= "" then
                            ...
                            MachoMenuNotification("Katana", "Copied.", 3)
                        end
                        return
                    end
                end
...
Citizen.CreateThread(
    function()
        ...
        MachoMenuNotification("KATANA 999", "Enjoy Katana.", 5)
    end
